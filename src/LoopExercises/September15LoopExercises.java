...
public class September15LoopExercises {
    ...
    public static int sum3or5(int n) {
        int sum = 0;
        for ( int i = 0; i < n; i++){ // for loops in java with initializer, loop condition, and loop variable update
            if (i % 3 == 0 || i % 5 == 0){
                sum = sum + i;
            }
        }
     ...
     * hint::: think first number in last number out (reverse the entry number then check if its the same as original)
     */
